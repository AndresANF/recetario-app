...
export const CreateRecipeScreen = ({ navigation }) => {
  ...
  const handleCreate = async () => {
    ...
    if (validIngredients.length === 0 || validSteps.length === 0) {
      Alert.alert('Campos incompletos', 'Para crear la receta debes añadir al menos un ingrediente y un paso.');
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <TextInput style={styles.input} placeholder="Ej: Arepas Rellenas" value={title} onChangeText={setTitle} />
        ...
        {ingredients.map((ing, index) => (
          <View key={`ing-${index}`} style={styles.dynamicRow}>
            ...
            <TextInput style={styles.dynamicInput} placeholder={`Ingrediente ${index + 1}`} value={ing} onChangeText={(text) => handleIngredientChange(text, index)} />
        ...
        {steps.map((step, index) => (
          <View key={`step-${index}`} style={styles.dynamicRow}>
            ...
            <TextInput style={[styles.dynamicInput, { minHeight: 60 }]} placeholder={`Paso ${index + 1}`} value={step} multiline={true} onChangeText={(text) => handleStepChange(text, index)} />
        ...
        <TouchableOpacity style={styles.submitBtn} onPress={handleCreate}>
          <Text style={styles.submitBtnText}>Guardar Receta</Text>
        </TouchableOpacity>
